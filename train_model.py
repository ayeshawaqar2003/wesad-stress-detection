#Final Breakthrough Accuracy: 71.52%
import os
import pickle
import numpy as np
...
def load_wesad_subject(subject_id):
    ...
    # Quantile Mapping - Best for Subject-Independent accuracy
...
# 3. BALANCING
print("⚖️ Applying SMOTE-Tomek to balance dataset and remove noise...")
smt = SMOTETomek(random_state=42)
X_train_bal, y_train_bal = smt.fit_resample(X_train, y_train)

# 4. TRAIN MODEL
print("\n--- Training Final ExtraTrees Ensemble ---")
...
plt.title(f"Confusion Matrix (Final Breakthrough Accuracy: {acc:.2%})")
plt.show()

# 7. SAVE
joblib.dump(model, 'wesad_stress_model.pkl')
print("\nModel saved as 'wesad_stress_model.pkl'.")
